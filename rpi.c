...
#include <watchdog.h>
/*Heart beat LED*/
unsigned int msg[] = {32, 0, 0x00038041, 8, 0, 130, 0, 0}; // Mailbox message buffer
const unsigned int MAILBOX = 0x3f00b880; // Mailbox Base address
volatile unsigned long num;
unsigned int count =15;

void delay();
void dummy();
void set(unsigned int led_set);
void mailbox_read();
void mailbox_write();

DECLARE_GLOBAL_DATA_PTR;

void open_led()                   // main function
{
	while(count--)
	{
		delay();
		set(1);            // Led on
		mailbox_write();   // Write message to mailbox
		mailbox_read();   // read message from mailbox
...
void set(unsigned int led_set)   // Intialising message
{
	msg[0]=32;           // Size of message buffer
	msg[1]=0;            // Request code
	msg[2]=0x00038041;   // Tag ID
	msg[3]=8;           // size of tag data
	msg[4]=0;           // request and response size
	msg[5]=130;        // port number
	msg[6]=led_set;   // led on/off
	msg[7]=0;         // end tag
}
...
void mailbox_write()
{
	...
	do
	{
		status = *(unsigned int *)(MAILBOX + 0x38); // accessing status register for write register
	}while(status & (unsigned int)0x80000000);          // checking if 31st bit of status register whether the write register is full
	*(unsigned int *)(MAILBOX + 0x20) = (((unsigned int )msg & ~(unsigned int)(0xf)) | (unsigned int)0x8);  // sending the address of message buffer to write register
}

void mailbox_read()
{
        volatile unsigned int status,buffer_msg;
	do
	{
		do
		{
			status = *(unsigned int *)(MAILBOX + (unsigned int)0x18);  // accessing status register for read register
		}while((status & (unsigned int)0x40000000));  // checking if 30th bit of status register whether the read register is empty
		buffer_msg = *(unsigned int *)(MAILBOX + 0x00);  // reading read register
	}while((buffer_msg & 0b1111) != (unsigned int)0x8);  // checking the last four bits(channel) from recieved response with the channel of transmitted value i.e; 8
}
/*Heart beat LED END*/
